"""
    使用 PyMuPDF 加载带有图片的 PDF 文档（适用单栏，无表格，无复杂数学公式）
...
def get_image_y_position(page, xref):
    """
    获取图片在页面上的垂直位置（Y坐标）
    返回值越小，位置越靠上（越靠前）
    """
...
def get_and_upload_images(page, page_num, doc_id):
    ...
    for img_index, img in enumerate(image_list, start=1):
        ...
        if base_image:
            ...
            # 获取图片位置
            y_pos = get_image_y_position(page, xref)
...
def extract_content_in_reading_order(page, page_num, doc_id):
    ...
    # 3. 按 Y 坐标排序（从上到下，模拟阅读顺序）
    content_blocks.sort(key=lambda x: x["y"])

    # 4. 把文本块和图像块连接成一个完整的字符串
    parts = []
...
if __name__ == '__main__':
    pdf_file = "../documents/doc_pdf/ViT模型详解.pdf"
